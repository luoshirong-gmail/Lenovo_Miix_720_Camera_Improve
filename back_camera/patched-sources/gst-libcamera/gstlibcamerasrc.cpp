...
struct GstLibcameraSrcState {
	...
	/* ⚠️ 2026-08-10 (无应用停流根治): 流活动标志 — task_run 在
	 * queueRequest 前检查; PAUSED 时 false → task 停在 pause (buffer-notify
	 * 的 task_resume 不会让它继续提交 — 之前 task_pause 软暂停被 resume
	 * 覆盖, IPA 持续扫描的回归根因)。PAUSED_TO_PLAYING 设 true 恢复。 */
	bool active_ = false;
...
int GstLibcameraSrcState::queueRequest()
{
	...
	for (GstPad *srcpad : srcpads_) {
		...
		if (ret != GST_FLOW_OK) {
			 ...
			 */
			/* ⚠️ 2026-08-10 (ENOBUFS 修复): 带控件的 request 被丢时,
			 * 控件放回 controls_ — 事件驱动控件不能随 ENOBUFS 消失 */
			if (!wrap->request_->controls().empty())
				controls_.retainControls(wrap->request_.get());
			return -ENOBUFS;
		}
...
gst_libcamera_src_task_run(gpointer user_data)
{
	 ...
	 * fatal error.
	 *
	 * ⚠️ 2026-08-10 (无应用停流根治): PAUSED 时 active_=false → 不提交
	 * request (task 停在 pause; buffer-notify 的 resume 不会让已暂停的
	 * 流继续 — 之前 task_pause 软暂停被 resume 覆盖的回归根因)。
	 */
	if (!state->active_)
		return;
...
gst_libcamera_src_change_state(GstElement *element, GstStateChange transition)
{
	...
	case GST_STATE_CHANGE_PAUSED_TO_PLAYING:
		/* ⚠️ 2026-08-10: active_=true 恢复流 (task_start 从暂停恢复,
		 * 不重新协商) */
		self->state->active_ = true;
		gst_task_start(self->task);
		break;
	case GST_STATE_CHANGE_PLAYING_TO_PAUSED:
		/* ⚠️ 2026-08-10 (无应用停流根治): active_=false 先于 task_pause —
		 * task_run 检查后不再提交 request (软暂停被 buffer-notify resume
		 * 覆盖的回归修复)。上游漏停 task — READY_TO_PAUSED 有
		 * gst_task_pause 但 PLAYING_TO_PAUSED 没有, 导致 PAUSED 时
		 * request 循环继续 (IPA 持续有帧 → 无应用时 continuous 对焦跑)。 */
		self->state->active_ = false;
		gst_task_pause(self->task);
		ret = GST_STATE_CHANGE_NO_PREROLL;
		break;
