			     ...
			     ControlInfoMap *ipaControls)
{
	...
	/* ⚠️ 2026-08-08 (全新): 注册 AF 控件 — 否则 libcamerasrc 的 capabilities_
	 * 检查把 AfMode/AfTrigger/LensPosition 当"不支持"忽略 (setProperty 直接
	 * 丢弃, 控件无法附加到 request → IPA queueRequest 收不到对焦控制) */
	...
	controls[&controls::LensPosition] = ControlInfo{ 0.0f, 2.0f, 0.0f };
	/* ⚠️ 2026-08-10 (AfState 完成检测): 声明 AfState — IPA 扫描状态机
	 * 实时输出 (Idle/Scanning/Focused), router 轮询检测扫描完成回
	 * manual (替代固定定时器, 官方语义) */
	controls[&controls::AfState] = ControlInfo{ controls::AfStateValues };
