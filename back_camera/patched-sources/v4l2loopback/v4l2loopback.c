...
/* control IDs */
/* ⚠️ 2026-08-09 (版本统一): base 0xf000 → 0xf900 — 与当前运行模块一致
 * (无条件注册版实测 keep_format=0x0098f900, af_trigger=0x0098f904)。
 * 官方上游 0.15.4 为 0xf000, 但运行模块自 04:45 无条件注册版起即 0xf900;
 * router.c 已按 0x0098f904 适配。统一所有源码副本防止内核升级重编译回退旧版。 */
#define V4L2LOOPBACK_CID_BASE (V4L2_CID_USER_BASE | 0xf900)
...
struct v4l2_loopback_device {
	...
	int focus_absolute;
	int af_trigger; /* CID_KEEP_FORMAT; lock the format, do not free
				   ...
				   enum v4l2_buf_type type)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		if (dev->keep_format)
			break;  /* allow CAPTURE when format is locked */
		/* HERMES-PATCH-2: writer streaming (output token held) + reader's */
		/* first S_FMT: allow it — try_fmt replaces fmt with the device */
		/* format, and vidioc_s_fmt_vid's HERMES-PATCH grants the token. */
		/* Without this, 2nd-open apps (qv4l2/guvcview) fail S_FMT with */
		/* EINVAL while the camera is live and show no preview. */
		if (has_other_owners(opener, dev))
			break;
			  ...
			  struct v4l2_requestbuffers *reqbuf)
{
	...
	/* MIN-3 patch: enforce minimum buffer allocation of 3.
	...
	MARK();
	opener->buffer_count = 0;
	/* IMAGE-ON-DEMAND patch: allocate when the device has no image yet, even
...
static int vidioc_streamon(struct file *file, void *fh, enum v4l2_buf_type type)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		if (dev->stream_tokens & token ||
		    !has_capture_token(opener->stream_token)) {
...
static int start_fileio(struct file *file, void *fh, enum v4l2_buf_type type)
{
	struct v4l2_loopback_device *dev = v4l2loopback_getdevice(file);
	struct v4l2_loopback_opener *opener = v4l2l_f_to_opener(file, fh);
	/* HERMES-PATCH-3 (2026-08-12 write 解耦可用组合): 用 used_buffer_count
...
static int v4l2_loopback_add(struct v4l2_loopback_config *conf, int *ret_nr)
{
	...
	/* ⚠️ ov5670-af-20260809 (根治): 无条件注册对焦控件 —
	 * 原条件 (dev->card_label 含 OV5670) 在 add 时偶发不满足
	 * (模块与源码行为不一致, 重启后必现) → 全部设备注册
	 * (router 只在 video16 使用, video99 等多余控件无副作用) */
