 ...
 *
 * Read the Grey World compensation coefficients from the tuning file.
 * yaml 示例 (ov5670.yaml):
 *   - Awb:
 *       redCompensation: 1.15
 *       blueCompensation: 1.10
 * 缺省 1.0 = 无补偿 (libcamera 原版行为).
 */
int Awb::init([[maybe_unused]] IPAContext &context, const YamlObject &tuningData)
{
	redCompensation_ = tuningData["redCompensation"].get<double>(1.0);
	blueCompensation_ = tuningData["blueCompensation"].get<double>(1.0);

	/* BNR: 缺省 imguCssBnrDefaults, yaml "bnr.lut" 32 值可覆盖 */
	bnr_ = imguCssBnrDefaults;
	if (tuningData.contains("bnr")) {
		const YamlObject &bnr = tuningData["bnr"];
		if (bnr.contains("lut")) {
			auto lutVals = bnr["lut"].getList<uint8_t>();
			if (lutVals && lutVals->size() == IPU3_UAPI_BNR_LUT_SIZE) {
				std::copy(lutVals->begin(), lutVals->end(),
					  bnr_.lut.values);
				LOG(IPU3Awb, Debug)
					<< "AWB BNR lut 已配置 (yaml): "
					<< static_cast<int>(bnr_.lut.values[0])
					<< ", " << static_cast<int>(bnr_.lut.values[1])
					<< ", ...";
			} else {
				LOG(IPU3Awb, Warning)
					<< "AWB BNR lut 需 " << IPU3_UAPI_BNR_LUT_SIZE
					<< " 值, 忽略 (got "
					<< (lutVals ? lutVals->size() : 0) << ")";
			}
		}
	}

	LOG(IPU3Awb, Debug) << "AWB 补偿系数: red " << redCompensation_
			    << " blue " << blueCompensation_;
	return 0;
}
...
constexpr uint16_t Awb::gainValue(double gain)
{
	 ...
	 *
	 * [CLAMP-REVERT 2026-08-11] wb_gains 是 __u16 (无符号), 负值会回绕成
	 * 8.9 倍增益 → 画面爆炸/崩溃. 保持 [0, 65535] (gain<1 → 0 → 1.0 倍).
	 * 白纸偏红 (R 强需降增益) 走 CCM (s16 有符号矩阵) 而非 wb_gains.
	 */
...
void Awb::awbGreyWorld()
{
	...
	/*
	 * [AWB-COMP] Grey World 统计偏差补偿 (tuning 配置化, 2026-08-11):
	 * 实测标定: 暗场景 (Y70) 无补偿 V-7.5 (绿) 需 k≈1.5; 亮场景 (Y120)
	 * k=1.5 过校正 V+7.8 (红) 需 k≈1.19 → 统计偏差场景依赖, 取温和
	 * 系数 (ov5670.yaml: redCompensation 1.15 / blueCompensation 1.10).
	 * 非线性: 仅放大 gain>1 (gain<1 保持 — BNR wb_gains 是 u16 无法
	 * 表达降增益, 白纸降 R 需走 CCM). 调参只改 yaml, 无需重编译.
	 */
	const auto compensateRed = [this](double g) {
		return g > 1.0 ? 1.0 + (g - 1.0) * redCompensation_ : g;
	};
	const auto compensateBlue = [this](double g) {
		return g > 1.0 ? 1.0 + (g - 1.0) * blueCompensation_ : g;
	};
	asyncResults_.redGain = compensateRed(redGain);
	asyncResults_.blueGain = compensateBlue(blueGain);
}
