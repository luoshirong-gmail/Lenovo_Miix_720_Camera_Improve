# OV5670 自动对焦修复记录 (2026-08-08, 更新至 2026-08-12)

Lenovo MIIX 720 (80VV) 后置 OV5670 (IPU3 管线) 自动对焦功能增强与修复。
基于 libcamera 0.7.0 (Ubuntu) 源码修改，改动以补丁形式归档于
`back_camera/ipa-patches/`。

> 2026-08-12 补充: **省电对焦** — 无应用时 (cam PAUSED) router 的 AF 控件
> 轮询从 100ms 降频到 1s (`AF_POLL_IDLE_MS`)。cam PAUSED 时无帧流入 IPA,
> continuous 重扫无意义, 降频省 CPU (~90%); 有应用时恢复 100ms 全速。
> （曾尝试 libcamerasrc 双流 viewfinder 驱动 AF, 帧率大跌, 已回退单流。）
